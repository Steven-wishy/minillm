...
logger = logging.getLogger("AGI_Pipeline")
...
def load_model_with_fallbacks(model_name: str, max_seq_length: int) -> Tuple[Any, Any]:
    """
...
def run_symmetrical_agi_pipeline(custom_dataset_path: str = None):
    max_seq_length = 32768  # Native maximum 32k context capability of LFM base configuration
    import os
    model_name = os.environ.get("MODEL_NAME", "LiquidAI/LFM2.5-230M-Base")
    ...
    model, tokenizer = load_model_with_fallbacks(model_name, max_seq_length)
    ...
    split_boundary = int(len(raw_dataset) * 0.85)
    ...
    end_idx = os.environ.get("END_IDX", str(len(train_dataset)))
    
    # Fully dynamic dataset slice usage
    training_slice = train_dataset[start_idx:int(end_idx)]
    ...
    for idx, sample in enumerate(training_slice):
        prompt_text = f"{sample['system_prompt']}\nUser: {sample['question']}\nAssistant: <think>"
        
        completions = []
        scores = []
        
        # Symmetrical collection of multi-turn debug trajectories
        for c in range(num_candidates):
            # Turn 1
            batch_inputs = tokenizer([prompt_text], return_tensors="pt").to("cuda")
            ...
            if passed:
                ...
                batch_inputs2 = tokenizer([turn2_prompt], return_tensors="pt").to("cuda")
    ...
    for idx, (prompt_text, completions, scores) in enumerate(batched_trajectories):
        try:
            ...
            loss_joint = loss_joint / accumulation_steps
            loss_joint.backward()
            
            if (idx + 1) % accumulation_steps == 0 or (idx + 1) == len(batched_trajectories):
                optimizer.step()
                optimizer.zero_grad()
    ...
    optimizer = AdamW(model.parameters(), lr=8e-6, weight_decay=0.05)
    model.train()
    optimizer.zero_grad()
    ...
    for idx, sample in enumerate(training_slice):
        try:
            ...
            loss_sft = loss_sft / accumulation_steps
            loss_sft.backward()
            
            if (idx + 1) % accumulation_steps == 0 or (idx + 1) == len(training_slice):
                optimizer.step()
                optimizer.zero_grad()
        except Exception as sample_err:
    ...
    print(f"[EVALUATION SUCCESS] Verification complete. Assertion success rate: {(passed_count / max(1, len(eval_slice))) * 100:.2f}%")
    print("="*80)
...
if __name__ == "__main__":
    print("Initializing Kaggle training script with Medium dataset from dataset mount...")
    model, tokenizer, search_adapter = run_symmetrical_agi_pipeline("/kaggle/input/minillm-medium-dataset/dataset_medium.json")
    print("Training, merging and Q4 export completed successfully.")
